...
async function loadCourseForBooking(courseId: string): Promise<CourseForBooking> {
...
async function vehicleHasBookingConflict(
	db: DbClient,
	vehicleId: string,
	start: Date,
	end: Date,
...
async function assertVehicleAvailableForBooking(
	...
	const hasConflict = await vehicleHasBookingConflict(db, vehicleId, start, end, {
...
async function findAvailableVehicleIdForStudentBooking(
	db: DbClient,
	instructorId: string,
	schoolId: string,
	start: Date,
...
async function createPracticeLessonForStudent(input: {
	...
	const { course, studentProfileId, instructorId, start, end, resolveVehicleId } =
		input;
