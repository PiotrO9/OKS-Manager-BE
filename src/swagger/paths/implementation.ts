...
const authLoginDataSchema = accessTokenDataSchema.extend({
	user: z.unknown(),
});

const instructorEventDtoSchema = z
	.object({
		id: uuidSchema,
...
const vehicleDataSchema = z.record(z.unknown());
...
function okDataSchema(description: string, dataSchema: z.ZodTypeAny) {
	return {
		description,
		content: {
			'application/json': {
