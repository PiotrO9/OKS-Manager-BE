...
**InstructorEvent** (`instructor_events`): blok czasu przypisany do instruktora; enum **`EventType`**: **`DRIVE`** (wymaga `vehicle_id`) | **`THEORY`**; opcjonalne **`capacity`** (limit uczestników); przypisania kursantów: **`event_participants`** (M:N `StudentProfile` ↔ event). API: [events-schedule-api.md](./events-schedule-api.md).
